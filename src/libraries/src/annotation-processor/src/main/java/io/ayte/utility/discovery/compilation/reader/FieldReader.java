...
public class FieldReader implements Reader {
    ...
    public SymbolCollection read(AccessibleSymbolCollection collection, Element element) {
        ClassElementReference<?> reference = ClassElementReader.convert(element.getEnclosingElement());
        return collection.addField(new FieldReference<>(reference, Types.getName(element)));
    }
